...
create_clock -name gtPllClkP    -period 4     [get_ports { gtPllClkP }]
...
create_generated_clock -name timingEmuRxClk \
    -divide_by 1 -add -master_clock timingRefClkDiv2 \
    -source [get_pins -hier -filter {name =~ U_App/U_TimingRx/U_rxUsrClk/I1}] \
    [get_pins -hier -filter {name =~ U_App/U_TimingRx/U_rxUsrClk/O}]
